...
- 当前配置：`vite.config.ts`、`wrangler.jsonc`、TypeScript 配置文件。
- 仓库目前仍接近初始模板。Tailwind CSS、shadcn/ui、Hono、Drizzle、D1 migrations、种子数据和 API 路由可能还需要在实现 MVP 前补齐。
...
- 代码注释只解释不直观的业务约束或技术原因，不写显而易见的注释。
